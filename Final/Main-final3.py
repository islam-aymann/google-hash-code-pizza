...
# In[1]:
    ...
    def overlapCondition(self):
        ...
        print(" Length "+str(len(self.endPoints)))
        if len(self.endPoints) == 0:
            print("ana fiii el overlap")
...
#            self.start = self.nextStart()
#            return
        self.maximum = self.endPoints[self.temp_areas_abs.index(max(self.temp_areas_abs))]
        print("  maximum area endPoint " + str(self.maximum))
        # overlap Condition
        for i in range(self.start[0], self.maximum[0]+1):
            print(i)
            for j in range(self.start[1], self.maximum[1]+1):
                print(j)
                print("################################################")
                print(self.checking[i][j])
                print("#############----------------###################")

                if self.checking[i][j] == 1: 
                    print("ana fi el overlap")                       
                    if len(self.endPoints) == 0:
                        print("ana fiii el overlap")
                        self.maxCells = self.maxCells - 1
                        if self.maxCells >= self.minCells:
                    ...
                    temp=self.endPoints.index(self.maximum)
                    self.endPoints.pop(temp)
                    self.temp_areas_abs.pop(temp)
                    print("ahlaaan")
...
# In[1]: # Range 0-->(MaxCells - MinCells) Condition
        ...
        if len(self.endPoints) == 0: # checking if all slices were not in the range / don't satisfy the min l condition per slice
            print("L Condition")
...
# In[1]: OverLapCondition           
        ...
        while self.overlapCondition():
            print("OV ---_________")
            if self.start != self.temp: # to restart the while Loop if we chosed anew start in end_points()
                return
            self.end_points()
            return (self.endPoints[self.temp_areas_abs.index(max(self.temp_areas_abs))])
# In[1]:    return (self.endPoints[self.temp_areas_abs.index(max(self.temp_areas_abs))])
        print("OOOOOOOOOO")
...
# In[1]: 
    ...
    def Area(self):
        ...
        self.out.write(str(self.start[0])+" "+str(self.start[1])+" "+str(self.endshape[0])+" " +str(self.endshape[1])+"\n")
        self.slices +=1
